...
and g' oc = function (* 各命令のアセンブリ生成 (caml2html: emit_gprime) *)
  ...
  | NonTail(x), ILd(Id.L(y)) -> Printf.fprintf oc "\tilw\t%s %s %s\n" reg_zero x y
  | NonTail(x), ILdF(Id.L(y)) -> Printf.fprintf oc "\tilw.s\t%s %s %s\n" reg_zero x y
  ...
  | NonTail(x), Ld(y, i) -> Printf.fprintf oc "\tlw\t%s %s %d\n" y x i
  | NonTail(_), St(x, y, i) -> Printf.fprintf oc "\tsw\t%s %s %d\n" y x i
  ...
  | NonTail(x), Floor(y) -> Printf.fprintf oc "\tfloor.w.s\t%s %s\n" y x;
  ...
  | NonTail(x), LdF(y, i) -> Printf.fprintf oc "\tlw.s\t%s %s %d\n" y x i
  | NonTail(_), StF(x, y, i) -> Printf.fprintf oc "\tsw.s\t%s %s %d\n" y x i
  ...
  | Tail, (Set _ | SetL _ | Mov _ | Neg _ | Ftoi _ | In _ | Fin _ | Out _
           | Add _ | AddI _ | Sub _ | Mul _ | Div _
           | SLL _ | SLLI _ | Ld _ | ILd _ as exp) ->
      g' oc (NonTail(regs.(0)), exp);
      Printf.fprintf oc "\tretl\n";
  | Tail, (FMov _ | FNeg _ | Itof _ | FSqrt _ | Floor _ | FAdd _ | FSub _ | FMul _ | FDiv _
           | LdF _ | ILdF _  as exp) ->
...
let f oc memtbl tp (Prog(mems, data, fundefs, e)) =
  ...
  let stack_init = Id.genid "stack_init" in
