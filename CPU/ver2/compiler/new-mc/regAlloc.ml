...
let rec g dest cont regenv = function (* 命令列のレジスタ割り当て (caml2html: regalloc_g) *)
  ...
  | Let((x, t) as xt, exp, e) ->
     (* 束縛変数xがすでにregenvにあるというのは *)
     (* alpha conversion が失敗していることを意味する *)
     assert (not (M.mem x regenv));
...
and g' dest cont regenv = function (* 各命令のレジスタ割り当て (caml2html: regalloc_gprime) *)
    (* これらの命令はレジスタを使わない *)
  | Nop | Set _ | SetL _ | Comment _ | Restore _ | ILd _ | ILdF _ as exp ->
     (Ans(exp), regenv)
  ...
  | LdF(x, i) -> (Ans(LdF(find x Type.Int regenv, i)), regenv)
  | StF(x, y, i) -> (Ans(StF(find x Type.Float regenv, find y Type.Int regenv, i)), regenv)
  ...
  | FIf(cmp,x,y,e1,e2) as exp ->
     g'_if dest cont regenv exp
       (fun e1' e2' -> FIf(cmp, find x Type.Float regenv, find y Type.Float regenv, e1', e2')) e1 e2
...
let f tp (Prog(mems, data, fundefs, e)) = (* プログラム全体のレジスタ割り当て (caml2html: regalloc_f) *)
