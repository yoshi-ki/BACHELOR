...
let rec infer_expr tenv exp =
  ...
  | ELetRec (e1,e2,e3,e4) ->
	 ...
	 let newtenv2 = [(e1,(TyFun(newvar1,newvar2)))] @ tenv in
...
let rec infer_cmd tenv cmd =
  ...
  | CExp e1 ->
	...
	(ty_subst ans (fst(infer)),(fst(infer))))
  | CDecl (e1,e2) ->
  ...
  | CRecDecl (e1,e2,e3) ->
    (let newvar1 = TyVar (new_tyvar()) in
	 ...
	 let ans = unify (snd(infer)) in
	 (ty_subst ans (fst(infer)),hikisuunokata))
...
let rec eval_command env tenv c =
  ...
  | CDecl (e1,e2) -> (print_type(fst(type_result)); print_newline(); (e1, (extend e1 (eval_expr env e2) env), (union [(e1, (fst(type_result)))] tenv), eval_expr env e2))
  | CRecDecl (f, x, e) ->
  	let oenv = ref [] in
	...
	Printf.printf "%s" "->";
	print_type(snd(type_result));
	print_newline();
	("-", (!oenv), (union [(f, TyFun((snd(type_result)),fst(type_result)))] tenv), VFun(x,e,oenv))	
  )
