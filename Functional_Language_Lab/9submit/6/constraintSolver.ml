...
let rec ty_subst sigma t =
	...
	| TyRef (t1) -> TyRef(ty_subst sigma t1)
...
let rec include_check alpha t =
    ...
    | TyPair(a,b) -> (include_check alpha a) || (include_check alpha b)
	| _ -> false
...
let rec unify const =
    ...
    | (TyVar(alpha),t) :: rest -> if (include_check alpha t) then raise TyError(Printf.printf "\027[38;5;1mType Error\027[0m";) else compose (unify (subst [(alpha,t)] rest)) [(alpha,t)]
    | (t,TyVar(alpha)) :: rest -> if (include_check alpha t) then raise TyError(Printf.printf "\027[38;5;1mType Error\027[0m";) else compose (unify (subst [(alpha,t)] rest)) [(alpha,t)]
	| (TyPair(a,b), TyPair(c,d)) :: rest -> unify (union [(a,c);(b,d)] rest)
	| (TyList(a), TyList(b)) :: rest -> unify (union [(a,b)] rest) 
	| (a,b) :: rest -> raise TyError(Printf.printf "\027[38;5;1mError\027[0m";Printf.printf ": ";Printf.printf "This expression has type ";print_type(b); Printf.printf "but an expression was expected of type "; print_type(a))
...
let rec get_type_vars ty =
	...
	| TyPair(a,b) -> (get_type_vars a) @ (get_type_vars b)
