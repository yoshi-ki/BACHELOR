...
let rec eval_expr env e =
  ...
  | EVar x ->
    (try
       let Thunk (e_thunk, env_thunk) = lookup x env in
	   eval_expr (env_thunk) (e_thunk)
     with
  ...
  | EApp (e1, e2) ->
    (match (eval_expr env e1) with
	 | VFun(x1,expr1,env_thunk) ->
	   let thunk1 = Thunk (e2,env) in
	   eval_expr (extend x1 thunk1 env_thunk) expr1 
	 | VRecFun (f,x,expr1,env_thunk) ->
	   let thunk1 = Thunk (ERecFun (f,x,expr1),env_thunk) in
	   let thunk2 = Thunk (e2,env) in
	   eval_expr (extend x thunk2 (extend f thunk1 env_thunk)) expr1)
  | ERecFun (f,x,exp) ->
  	  VRecFun(f,x,exp,env)
  | ELetRec (f, x, e1, e2) ->
  	  let newenv = extend f (Thunk(ERecFun(f,x,e1), env)) env
	  in
	  eval_expr newenv e2
  | EPair (e1,e2) ->
...
let rec eval_command env tenv c =
  ...
  | CDecl (e1,e2) -> (print_type(fst(type_result));print_newline();(e1, (extend e1 (Thunk (e2,env)) env), (snd(type_result)), eval_expr env e2))
  | CRecDecl (f, x, e) ->
  	let newenv = extend f (Thunk(ERecFun(f,x,e),env)) env
  	in
