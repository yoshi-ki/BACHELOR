...
int main() {
  ...
  while(searchNode(list, dictionary[N])->beforenode != 0){
    if (searchNode(list, dictionary[N])->beforenode == -1){flag = 0; break;}
    N = searchNode(list, dictionary[N])->beforenode;
    answer[answer_num] = N;
    answer_num++;
    if (answer_num > 1000) {flag = 0; break;}
  }
  if (flag){
    printf("%s\n",dictionary[0]);
